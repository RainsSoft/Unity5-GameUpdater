...
internal class BundleManager
{
    ...
    public void CopyAll(bool copyToResources = false)
    {
        ...
        {
            //目标被删除了，里面的所有文件都没了,其public/vc文件也就不存在了,后面的bundle.isExist()这个判断逻辑有问题
        ...
        foreach (var bundle in bundles)
        {
            ...
            //string childFolderPath = destPath + "/" + bundle.name;
            if (childFolder > 0)
            {
			...
			if(!bundle.isExist() || copyToResources){
                SaveBytesToBundle(destPath + "/" + bundle.name/*destPath + "/" +Config.versionFileName*/, bytes);//??这是什么鬼？
			}
        }
    }
